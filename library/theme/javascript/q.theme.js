...
Plugin:     Q Theme
Version:    2.9.4
Date:       16/01/2019 02:30:43 pm
*/
$q_modal_hash_value=!1;$q_modal_key=!1;$q_modal_args=!1;$q_modal_content=!1;if(typeof jQuery!=='undefined'){jQuery(document).ready(function(){jQuery(window).bind('hashchange',function(e){e.preventDefault();q_modal_toggle($q_modal_args)})})}
...
jQuery('html').removeClass('modal-open');jQuery('.featherlight').removeClass('modal-'+$q_modal_key);if($q_modal_args.shift){jQuery('[data-modal-key="'+$q_modal_key+'"]').html($q_modal_content)}
window.location.hash=$hash;return!1}
function q_modal_toggle($q_modal_args)
{$q_modal_key=q_modal_hash();jQuery('.featherlight').remove();if(!$q_modal_key){return!1}
q_modal_callback($q_modal_args,$q_modal_key);jQuery('html').addClass('modal-open');$q_modal_content=jQuery('[data-modal-key="'+$q_modal_key+'"]').html();if($q_modal_args.shift){jQuery('.hours_gravity_form').empty()}
$q_modal_close=jQuery.featherlight($q_modal_content,{type:'html',beforeOpen:q_modal_get_device,afterClose:q_modal_close,afterOpen:q_do_lazy,variant:'modal-'+$q_modal_key})}
...
function q_load_youtube(e){$youtube=jQuery('[data-youtube]').attr('data-youtube');if(!$youtube){console.log('No youtube selector found.');return!1}
var youtube_id=q_get_youtube_id($youtube);var iframeMarkup='<iframe width="560" height="315" src="//www.youtube.com/embed/'+youtube_id+'?rel=0&showinfo=0&autoplay=1" frameborder="0" allowfullscreen></iframe>';e.parent(".q-youtube").html(iframeMarkup)}
...
jQuery(window).bind("load",function(){$the_hash=q_toggle_hash();if($the_hash)q_toggle($the_hash)});jQuery(document).ready(function(){jQuery(window).bind('hashchange',function(e){history.navigationMode='compatible';e.preventDefault();$the_hash=q_toggle_hash();if($the_hash)q_toggle($the_hash)})});function q_toggle(data_id){if(jQuery("[data-toggle-content='"+data_id+"']").length){var did_we_toggle=!1;jQuery('.q-toggle-content').each(function(){if(jQuery(this).is(":visible")){jQuery(this).find('p').animate({opacity:'0'},300,function(){jQuery(this).parent().slideUp(200).addClass('hidden')})}});if(did_we_toggle){setTimeout(function(){jQuery("[data-toggle-content='"+data_id+"']").removeClass("hidden").slideDown(500,function(){jQuery(this).find('p').delay(200).animate({opacity:1},900)})},550)}else{jQuery("[data-toggle-content='"+data_id+"']").removeClass("hidden").slideDown(500,function(){jQuery(this).find('p').delay(200).animate({opacity:1},900)})}
jQuery(".q-toggle-trigger").parent('*').removeClass('current');jQuery("#toggle-nav-"+data_id).parent('*').addClass('current')}}
